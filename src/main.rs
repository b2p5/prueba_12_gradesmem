...
struct TransactionInfo {
    size: u64,      // Tamaño de la transacción
    weight: u64,    // Peso de la transacción
}
...
impl MempoolInfo {
    // // Crea una nueva instancia de MempoolInfo
    ...
    pub fn insert(&mut self, txid: String, transaction: TransactionInfo) {
        self.transactions.insert(txid, transaction);
    }

    // Elimina una transacción del mempool
    // pub fn delete(&mut self, txid: &str) -> Option<TransactionInfo> {
...
fn rocket() -> _ {
    ...
    // let mempool_info = Arc::new(Mutex::new(MempoolInfo::new()));
    // let mempool_info_clone = Arc::clone(&mempool_info);
...
// fn get_raw_transaction(client: &Client, txid: &str) -> Result<TransactionInfo, BitcoinRpcError> {
//     match client.call::<Value>("getrawtransaction", &[Value::String(txid.to_string()), Value::Bool(true)]) {
//         Ok(raw_tx) => {
//             let size = raw_tx["size"].as_u64().unwrap_or(0);
//             let weight = raw_tx["weight"].as_u64().unwrap_or(0);
//             Ok(TransactionInfo { size : size, weight : weight })
//         },
//         Err(e) => Err(e)
//     }
// }
        ...
        // Itera mempool_txs para calcular el peso y tamaño de cada transacción
        // for hash_tx in mempool_txs.clone() {
        //     let tx_result = get_raw_transaction(&client, &hash_tx);
        
        //     match tx_result {
        //         Ok(tx_info) => {
        //             mempool_info_clone.lock().unwrap().insert(hash_tx, tx_info);
        //         },
        //         Err(e) => {
        //             // Aquí puedes manejar el error como prefieras
        //             eprintln!("Error al obtener la transacción:{:?}", hash_tx);
        //             continue;
        //         }
        //     }
        // }
